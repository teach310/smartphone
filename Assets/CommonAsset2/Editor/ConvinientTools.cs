	...
	public class ConvinientTools : EditorWindow {
		...
		async void OnGUI () {
			if (GUILayout.Button ("AnyTest")) {
				...
				Debug.LogFormat ("ItemId {0}", keyValuePair.id);
			}

			if (GUILayout.Button ("CDTest")) {
				var classInfoSet = await new MasterDataLoader ().GetClassInfoSetAsync (MasterDataSettings.Instance.masterDataUrl);
